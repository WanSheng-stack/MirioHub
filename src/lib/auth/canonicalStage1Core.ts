/**
 * Canonical Stage-1 contract (pure). No `server-only` so unit tests can import
 * normalize/hash without the production boundary. Production code must still
 * import through `@/lib/auth/canonicalStage1`, which re-exports these symbols
 * behind `server-only`.
 *
 * raw client input ≠ canonical payload.
 * Illegal numerics and unknown transport modes are REJECTED, never dropped.
 */
