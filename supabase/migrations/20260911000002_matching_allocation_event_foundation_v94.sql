-- PHASE 6.7A.3 / 6.7A.3A / 6.7A.3B — allocation and event foundation (v94)
...
-- Constraint/index expected defs are frozen deployed-catalog forms from
-- pg_get_constraintdef(oid, false) / pg_get_indexdef(oid, 0, false).
-- Compare with whitespace-only canonicalize; do not strip parentheses or
-- rewrite IN / ANY / ALL / BETWEEN.
...
-- Six tables: five fulfillment foundations plus safety_checklist_acceptance_items.
-- Checklist items are a child table. There is no confirmed_items text[] and no
-- pairwise array uniqueness CHECK. item_order 1-64 UNIQUE per acceptance caps
-- cardinality at 64. non-empty checklist items is a future transactional writer
-- invariant; DDL does not guarantee a header always has a child row.
-- contract_events.event_payload CHECK blocks listed sensitive keys at the top
-- level only. The future unique writer must recursively inspect the JSON tree.
-- Actor-is-contract-participant is not a single-table CHECK; the future writer
-- must re-read the contract parties.
BEGIN;
...
COMMENT ON COLUMN public.contract_events.event_payload IS
  'JSON object only. The table CHECK rejects listed sensitive keys at the top level only. It does not recursively inspect nested objects. The future unique writer must walk the full JSON tree for unknown, private, contact, identity, location, and fee keys. Browser has no write privilege; the table stays empty until that writer exists.';
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 5. safety_checklist_acceptances — confirmation header only, no media
-- ═══════════════════════════════════════════════════════════════════════════
...
COMMENT ON TABLE public.safety_checklist_acceptances IS
  'Header for one versioned safety-checklist confirmation at a stage. Item keys live in safety_checklist_acceptance_items. No photos, files, user prose, or a field claiming there is no unknown risk. Residual-risk copy belongs in the versioned checklist content. Actor-is-contract-participant is not enforced by this table CHECK; the future writer must re-read contract parties. non-empty checklist items is a future transactional writer invariant.';
...
COMMENT ON COLUMN public.safety_checklist_acceptances.actor_user_id IS
  'Confirming user. Cross-table membership on the contract is a future writer invariant, not a single-table CHECK.';

-- ═══════════════════════════════════════════════════════════════════════════
-- 5b. safety_checklist_acceptance_items — normalized item keys
-- ═══════════════════════════════════════════════════════════════════════════

CREATE TABLE public.safety_checklist_acceptance_items (
  acceptance_id uuid NOT NULL
    CONSTRAINT safety_checklist_acceptance_items_acceptance_id_fkey
      REFERENCES public.safety_checklist_acceptances(id)
      ON UPDATE RESTRICT
      ON DELETE RESTRICT,
  item_key text NOT NULL,
  item_order smallint NOT NULL,
  created_at timestamptz NOT NULL DEFAULT timezone('utc', now()),
  CONSTRAINT safety_checklist_acceptance_items_pkey
    PRIMARY KEY (acceptance_id, item_key),
  CONSTRAINT safety_checklist_acceptance_items_acceptance_id_item_order_key
    UNIQUE (acceptance_id, item_order),
  CONSTRAINT safety_checklist_acceptance_items_item_order_check
    CHECK (item_order BETWEEN 1 AND 64),
  CONSTRAINT safety_checklist_acceptance_items_item_key_check
    CHECK (
      item_key = btrim(item_key)
      AND char_length(item_key) BETWEEN 1 AND 100
      AND item_key ~ '^[a-z0-9][a-z0-9_.:-]*$'
    )
);

COMMENT ON TABLE public.safety_checklist_acceptance_items IS
  'Normalized checklist item keys for one acceptance. UNIQUE (acceptance_id, item_order) plus item_order BETWEEN 1 AND 64 caps a confirmation at 64 items. UNIQUE (acceptance_id, item_key) forbids duplicate keys in one confirmation. non-empty checklist items is a future transactional writer invariant; DDL does not require a header to have a child row. No photos, files, phones, WhatsApp/Viber values, address, GPS, plates, codes, identity documents, or evidence prose.';

COMMENT ON COLUMN public.safety_checklist_acceptance_items.item_key IS
  'Stable machine key only. Trimmed, 1-100 chars, ^[a-z0-9][a-z0-9_.:-]*$. Never a phone, message, or free-text sentence.';

COMMENT ON COLUMN public.safety_checklist_acceptance_items.item_order IS
  '1-based order unique per acceptance. Range 1-64 is the cardinality cap.';
...
ALTER TABLE public.safety_checklist_acceptance_items ENABLE ROW LEVEL SECURITY;
...
REVOKE ALL ON TABLE public.safety_checklist_acceptance_items FROM PUBLIC;
REVOKE ALL ON TABLE public.safety_checklist_acceptance_items FROM anon;
REVOKE ALL ON TABLE public.safety_checklist_acceptance_items FROM authenticated;
REVOKE ALL ON TABLE public.safety_checklist_acceptance_items FROM service_role;
